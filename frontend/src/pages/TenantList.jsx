...
export default function TenantList() {
  ...
  useEffect(()=>{
    ...
    .then(r=>setTens(r.data)).catch(console.error);
  },[]);
  return (
    <div className="p-4">
      <div className="flex justify-between mb-4">
        <h1 className="text-2xl">Tenants</h1>
        <button onClick={()=>nav('/tenants/new')}
          className="bg-green-500 text-white px-3 py-1 rounded">+ Novo</button>
      </div>
      <table className="w-full table-auto border">
        <thead className="bg-gray-200"><tr>
          <th className="p-2">ID</th><th className="p-2">Nome</th>
          <th className="p-2">Email</th><th className="p-2">Plano</th>
          <th className="p-2">Ações</th>
        </tr></thead>
        <tbody>
          {tens.map(t=>(
            <tr key={t._id} className="border-t">
              <td className="p-2 font-mono">{t._id}</td>
              <td className="p-2">{t.name}</td>
              <td className="p-2">{t.contactEmail}</td>
              <td className="p-2">{t.plan}</td>
              <td className="p-2">
                <button onClick={()=>nav(`/tenants/${t._id}`)}
